...
export function GitHubPublisher({ projectTitle = "ProjectForge AI — AI Project Architect", onRepoPublished }: GitHubPublisherProps) {
  const { githubState } = useGitHub();

  const [repoName, setRepoName] = useState("ProjectForge_AI");
  const [description, setDescription] = useState("ProjectForge AI — AI Project Architect & Rescue Mentor (V2 Auto-Ship)");
  const [isPrivate, setIsPrivate] = useState(false);

  const [publishing, setPublishing] = useState(false);
  const [stepIndex, setStepIndex] = useState(0);
  const [publishedData, setPublishedData] = useState<{ repositoryUrl: string; commitSha: string } | null>({
    repositoryUrl: "https://github.com/Sky-ydv2008/ProjectForge_AI",
  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const targetUrl = "https://github.com/Sky-ydv2008/ProjectForge_AI";
